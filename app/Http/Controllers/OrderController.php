            ...
            if (isset($warehouse_order->GetOrderShipmentInfoResult->OrderShipmentInfo)) {
                ...
                if ($warehouse_order->OrderStatus == 4 && $orders->order->fulfillment_status == null && isset($warehouse_order->Shipments->ShipmentDetail)) {
                    ...
                    foreach ($warehouse_shipment as $shipment) {
                        ...
                        //dd($item_ids_array);
                        try {
                            $shopify_result = $shopify->call(['URL' => 'orders/' . $id . '/fulfillments.json', 'METHOD' => 'POST', "DATA" => ["fulfillment" => array("location_id" => $locations->locations[0]->id, "tracking_number" => $shipment->TrackingNumber, "line_items" => $item_ids_array, "notify_customer" => true,"service"=>"Warehouse.Space")]]);
                        } catch (\Exception $e) {
